...
bool isElementInArray(int element, int array[]){
...
int main() {
	...
	for (int i = 0; i < 5; i++) {
		...
		} while (isElementInArray(temporaryLotteryResult, lotteryResults));
		...
		while (!std::cin.good() || temporaryLotteryGuess < 1 || temporaryLotteryGuess>20 || (temporaryLotteryGuess - floor(temporaryLotteryGuess) != 0.0f) || isElementInArray(temporaryLotteryGuess, lotteryBet)) { //This checks if the guessber is an integer in the given range
			std::cin.clear();
	...
	std::cout << "\nLottery results ";
	for (int i = 0; i < 5; i++) {
		if (isElementInArray(lotteryBet[i],lotteryResults)) winnings += 10;
